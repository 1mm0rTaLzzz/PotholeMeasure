...
# Examples:
#   .\run_all.ps1
#   .\run_all.ps1 -SkipDataPrep -SkipFinetune
...
# ----- 1. Data prep ----------------------------------------------------------
if (-not $SkipDataPrep) {
    Step ("1/6 prepare_data (" + $RddRoot + " to data\)")
    $maskFlag = @()
...
if (-not $SkipFinetune) {
    ...
    # Override epochs in the YAML before training.
...
if (-not $SkipAblation) {
    Step ("5/6 relative_ablation (out: " + $ablOut + ")")
...
$evalCmd = "python scripts/evaluate.py --gt data\annotations\test_gt.json --output " + $Out + "\evaluate.csv --latex"
Write-Host ("then run: " + $evalCmd) -ForegroundColor Yellow
